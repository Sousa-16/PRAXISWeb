...
1. Never commit `api/.env`, `.local-secrets/`, `*.key`, proxy secrets, or DuckDNS tokens.
2. SSH: key-only login (no password). Keep the private key outside the repo (for example a local `.local-secrets/` folder with mode `600`, which is fully gitignored). Optional: `fail2ban` on port 22.
